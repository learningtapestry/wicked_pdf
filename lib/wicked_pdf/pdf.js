  ...
  const createPdf = async (options) => {
    ...
    try {
      ...
      if (options.useFirefox) {
        ...
        // Check if the Chrome is not installed in the default location,
        // we defined the path based on the environment variable.
        const chromePath = puppeteer.executablePath();
        if (!chromePath || chromePath.trim() === '') {
          browserParams.executablePath = process.env.PUPPETEER_EXECUTABLE_PATH;
        }
